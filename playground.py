...
def predict(data: np.ndarray):
    model = MNIST_CNN.load_weights(Path("ckpt.pkl"))
    pred, conf_ = model.predict_with_ndarray(data)
    conf = f"{conf_ * 100:.2f}%"
    return pred, conf
...
def train():
    # 학습용 데이터 로드
    train_images, train_labels = load_mnist_by_kind(Path("dataset"), "train")
...
def test():
    ...
    pred, conf = predict(image_np)
    return pred, conf


def main(image: ndarray):
    model = MNIST_CNN.load_weights(Path("ckpt.pkl"))
    pred, conf = model.predict_with_ndarray(image)
    return pred, conf
